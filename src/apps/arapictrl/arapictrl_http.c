...
static int verify_auth_token(const char *token, AuthSession *out_sess) {
    ...
    ArapictrlConfig *cfg = arapictrl_config_get();
...
static void log_ctrl_audit(const char *user, const char *tenant, const char *action, const char *severity, int status, const char *details) {
    ArapictrlConfig *cfg = arapictrl_config_get();
    int fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd < 0) return;
    ...
    int body_len = snprintf(body, sizeof(body),
        "{\"user\":\"%s\",\"tenant\":\"%s\",\"service\":\"arctrl\",\"action\":\"%s\",\"severity\":\"%s\",\"status_code\":%d,\"details\":\"%s\"}",
        user ? user : "system", tenant ? tenant : "global", action, severity ? severity : "CRITICAL", status, details);
...
static void* http_client_worker(void *arg) {
    ...
    /* 1. GET /arapi/ctrl/status */
    if (strcmp(method, "GET") == 0 &&
        (strcmp(path, "/arapi/ctrl/status") == 0 || strcmp(path, "/arapi/ctrl/health") == 0 || strcmp(path, "/arapi/ctrl/") == 0)) {
        char json[512];
        snprintf(json, sizeof(json),
            "{\"status\":\"online\",\"backend\":\"arapictrl\",\"prefix\":\"/arapi/ctrl\","
            "\"control_plane\":\"ALRI Sovereign Master Infrastructure Control Plane\"}\n");
        send_http_response(client_fd, 200, "OK", "application/json", NULL, json);
        ar_socket_close(client_fd);
        return NULL;
    }

    // Authenticate token for all master control plane endpoints
    ...
    // STRICT ZERO-TRUST MASTER ACCESS GATE
    if (!sess.is_master) {
        log_ctrl_audit(sess.user, sess.tenant, "unauthorized_ctrl_access_attempt", "SECURITY", 403, "Master privileges required");
        send_http_response(client_fd, 403, "Forbidden", "application/json", NULL, "{\"error\":\"Acesso negado: Exclusivo para Holding Master (is_master=true)\"}\n");
        ar_socket_close(client_fd);
        return NULL;
    }

    /* 2. GET /arapi/ctrl/services */
    if (strcmp(method, "GET") == 0 && strncmp(path, "/arapi/ctrl/services", 20) == 0) {
        char *json = arapictrl_db_list_services_json();
        send_http_response(client_fd, 200, "OK", "application/json", NULL, json);
        free(json);
        ar_socket_close(client_fd);
        return NULL;
    ...
    /* 3. POST /arapi/ctrl/services/restart */
    if (strcmp(method, "POST") == 0 && strcmp(path, "/arapi/ctrl/services/restart") == 0) {
        char *service_id = extract_json_string(body, "service_id");
        if (!service_id) {
            send_http_response(client_fd, 400, "Bad Request", "application/json", NULL, "{\"error\":\"Campo 'service_id' obrigatório\"}\n");
            ar_socket_close(client_fd);
            return NULL;
        }

        int res = arapictrl_db_restart_service(service_id, sess.user);
        if (res == 0) {
            log_ctrl_audit(sess.user, sess.tenant, "service_restart", "CRITICAL", 200, service_id);
            send_http_response(client_fd, 200, "OK", "application/json", NULL, "{\"status\":\"success\",\"message\":\"Serviço reiniciado com sucesso\"}\n");
        } else {
            send_http_response(client_fd, 404, "Not Found", "application/json", NULL, "{\"error\":\"Serviço não encontrado\"}\n");
        }
    ...
    /* 4. GET /arapi/ctrl/system */
    if (strcmp(method, "GET") == 0 && strncmp(path, "/arapi/ctrl/system", 18) == 0) {
        char *json = arapictrl_db_get_system_metrics_json();
        send_http_response(client_fd, 200, "OK", "application/json", NULL, json);
        free(json);
        ar_socket_close(client_fd);
        return NULL;
    ...
    /* Fallback 404 */
    send_http_response(client_fd, 404, "Not Found", "application/json", NULL, "{\"error\":\"Endpoint not found in /arapi/ctrl\"}\n");
    ar_socket_close(client_fd);
    return NULL;
}
...
int arapictrl_http_server_start(const char *bind_ip, int port) {
    ...
    alri_print(GRN "[ARAPICTRL]" RST " Master Control Plane listening on %s:%d (prefix: /arapi/ctrl)\n", bind_ip ? bind_ip : "127.0.0.1", port);
    return 0;
}
...
void arapictrl_http_server_stop(void) {
    ...
    alri_print(CYN "[ARAPICTRL]" RST " Master Control Plane stopped.\n");
}
