...
static void handle_ipc_query(int fd, const char *payload, int payload_len) {
    ...
    if (strcmp(q, "status") == 0) {
        rlen = snprintf(resp, sizeof(resp),
            "[ARAPICTRL] Master Control Plane API Backend Status:\n"
            "  State: RUNNING\n"
            "  HTTP Port: %d (Active)\n"
            "  Route Prefix: %s\n"
            "  Storage: %s\n",
        ...
        rlen = snprintf(resp, sizeof(resp),
            "ARAPICTRL — Sovereign Master Control Plane Backend v1.0.0\n"
...
static void *arapictrl_ipc_thread(void *arg) {
    (void)arg;
    ArapictrlConfig *cfg = arapictrl_config_get();
    ...
    while (g_app_running) {
        ...
        snprintf(reg_frame, sizeof(reg_frame),
                 "arapictrl %s/* * * production proxy=http://127.0.0.1:%d",
...
int main(int argc, char **argv) {
    ...
    alri_print(BLD GRN "== [ARAPICTRL] Starting Sovereign Master Control Plane ==\n" RST);

    arapictrl_config_load("arapictrl.cfg");
    ArapictrlConfig *cfg = arapictrl_config_get();

    if (arapictrl_db_init(cfg->data_dir) != 0) {
        alri_print(RED "[ARAPICTRL] Failed to initialize control plane database\n" RST);
        return 1;
    }

    if (arapictrl_http_server_start(cfg->server_bind, cfg->server_port) != 0) {
        alri_print(RED "[ARAPICTRL] Failed to start HTTP server on port %d\n" RST, cfg->server_port);
        arapictrl_db_close();
        return 1;
    }

    void *ipc_t = ar_thread_create(arapictrl_ipc_thread, NULL);

    while (g_app_running) {
        ar_sleep_ms(250);
    }

    alri_print(YLW "[ARAPICTRL] Shutting down service...\n" RST);
    arapictrl_http_server_stop();
    arapictrl_db_close();

    if (ipc_t) ar_thread_join(ipc_t);

    alri_print(GRN "[ARAPICTRL] Clean shutdown completed.\n" RST);
    return 0;
}
