...
static void handle_ipc_query(int fd, const char *payload, int payload_len) {
    ...
    if (strcmp(q, "status") == 0) {
        rlen = snprintf(resp, sizeof(resp),
            "[ARAPICLOUD] Cloud Storage API Backend Status:\n"
            "  State: RUNNING\n"
            "  HTTP Port: %d (Active)\n"
            "  Route Prefix: %s\n"
            "  Storage: %s\n",
        ...
        rlen = snprintf(resp, sizeof(resp),
            "ARAPICLOUD — Sovereign Cloud Storage API Backend v1.0.0\n"
...
static void *arapicloud_ipc_thread(void *arg) {
    (void)arg;
    ArapicloudConfig *cfg = arapicloud_config_get();
    ...
    while (g_app_running) {
        ...
        snprintf(reg_frame, sizeof(reg_frame),
                 "arapicloud %s/* * * production proxy=http://127.0.0.1:%d",
...
int main(int argc, char **argv) {
    ...
    alri_print(BLD GRN "== [ARAPICLOUD] Starting Sovereign Cloud Storage Backend ==\n" RST);

    arapicloud_config_load("arapicloud.cfg");
    ArapicloudConfig *cfg = arapicloud_config_get();

    if (arapicloud_db_init(cfg->data_dir) != 0) {
        alri_print(RED "[ARAPICLOUD] Failed to initialize cloud storage database\n" RST);
        return 1;
    }

    if (arapicloud_http_server_start(cfg->server_bind, cfg->server_port) != 0) {
        alri_print(RED "[ARAPICLOUD] Failed to start HTTP server on port %d\n" RST, cfg->server_port);
        arapicloud_db_close();
        return 1;
    }

    void *ipc_t = ar_thread_create(arapicloud_ipc_thread, NULL);

    while (g_app_running) {
        ar_sleep_ms(250);
    }

    alri_print(YLW "[ARAPICLOUD] Shutting down service...\n" RST);
    arapicloud_http_server_stop();
    arapicloud_db_close();

    if (ipc_t) ar_thread_join(ipc_t);

    alri_print(GRN "[ARAPICLOUD] Clean shutdown completed.\n" RST);
    return 0;
}
