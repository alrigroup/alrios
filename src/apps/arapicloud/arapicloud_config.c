...
static ArapicloudConfig g_cfg;
static int g_loaded = 0;

static void set_defaults(void) {
    memset(&g_cfg, 0, sizeof(g_cfg));
    strncpy(g_cfg.app_id, "arapicloud", sizeof(g_cfg.app_id) - 1);
    strncpy(g_cfg.app_name, "ALRI Sovereign Cloud Storage & Drive API Backend", sizeof(g_cfg.app_name) - 1);
    g_cfg.server_port = 9695;
    strncpy(g_cfg.server_bind, "127.0.0.1", sizeof(g_cfg.server_bind) - 1);
    strncpy(g_cfg.route_prefix, "/arapi/cloud", sizeof(g_cfg.route_prefix) - 1);
    ...
    strncpy(g_cfg.data_dir, "storage/arcloud", sizeof(g_cfg.data_dir) - 1);
}
...
int arapicloud_config_load(const char *path) {
    set_defaults();
    if (!path) {
        g_loaded = 1;
        return 0;
...
ArapicloudConfig* arapicloud_config_get(void) {
