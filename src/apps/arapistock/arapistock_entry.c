...
static void handle_ipc_query(int fd, const char *payload, int payload_len) {
    ...
    if (strcmp(q, "status") == 0) {
        rlen = snprintf(resp, sizeof(resp),
            "[ARAPISTOCK] Inventory Management API Backend Status:\n"
            "  State: RUNNING\n"
            "  HTTP Port: %d (Active)\n"
            "  Route Prefix: %s\n"
            "  Storage: %s\n",
        ...
        rlen = snprintf(resp, sizeof(resp),
            "ARAPISTOCK — Sovereign Inventory API Backend v1.0.0\n"
...
static void *arapistock_ipc_thread(void *arg) {
    (void)arg;
    ArapistockConfig *cfg = arapistock_config_get();
    ...
    while (g_app_running) {
        ...
        snprintf(reg_frame, sizeof(reg_frame),
                 "arapistock %s/* * * production proxy=http://127.0.0.1:%d",
...
int main(int argc, char **argv) {
    ...
    alri_print(BLD GRN "== [ARAPISTOCK] Starting Sovereign Inventory Management Backend ==\n" RST);

    arapistock_config_load("arapistock.cfg");
    ArapistockConfig *cfg = arapistock_config_get();

    if (arapistock_db_init(cfg->data_dir) != 0) {
        alri_print(RED "[ARAPISTOCK] Failed to initialize inventory database\n" RST);
        return 1;
    }

    if (arapistock_http_server_start(cfg->server_bind, cfg->server_port) != 0) {
        alri_print(RED "[ARAPISTOCK] Failed to start HTTP server on port %d\n" RST, cfg->server_port);
        arapistock_db_close();
        return 1;
    }

    void *ipc_t = ar_thread_create(arapistock_ipc_thread, NULL);

    while (g_app_running) {
        ar_sleep_ms(250);
    }

    alri_print(YLW "[ARAPISTOCK] Shutting down service...\n" RST);
    arapistock_http_server_stop();
    arapistock_db_close();

    if (ipc_t) ar_thread_join(ipc_t);

    alri_print(GRN "[ARAPISTOCK] Clean shutdown completed.\n" RST);
    return 0;
}
