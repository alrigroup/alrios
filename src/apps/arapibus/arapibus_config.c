...
static ArapibusConfig g_cfg;
static int g_loaded = 0;

static void set_defaults(void) {
    memset(&g_cfg, 0, sizeof(g_cfg));
    strncpy(g_cfg.app_id, "arapibus", sizeof(g_cfg.app_id) - 1);
    strncpy(g_cfg.app_name, "ALRI Sovereign Business & HR Management API Backend", sizeof(g_cfg.app_name) - 1);
    g_cfg.server_port = 9670;
    strncpy(g_cfg.server_bind, "127.0.0.1", sizeof(g_cfg.server_bind) - 1);
    strncpy(g_cfg.route_prefix, "/arapi/bus", sizeof(g_cfg.route_prefix) - 1);
    ...
    strncpy(g_cfg.data_dir, "storage/arbus", sizeof(g_cfg.data_dir) - 1);
}
