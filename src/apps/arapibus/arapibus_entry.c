...
static void handle_ipc_query(int fd, const char *payload, int payload_len) {
    ...
    if (strcmp(q, "status") == 0) {
        rlen = snprintf(resp, sizeof(resp),
            "[ARAPIBUS] Business & HR API Backend Status:\n"
            "  State: RUNNING\n"
            "  HTTP Port: %d (Active)\n"
            "  Route Prefix: %s\n"
            "  Storage: %s\n"
            "  Zero-Trust Multi-Tenant RBAC: ACTIVE\n",
            cfg->server_port, cfg->route_prefix, cfg->data_dir);
    } else {
        rlen = snprintf(resp, sizeof(resp),
            "ARAPIBUS — Sovereign Corporate & HR Management API Backend v1.0.0\n"
...
static void *arapibus_ipc_thread(void *arg) {
    (void)arg;
    ArapibusConfig *cfg = arapibus_config_get();
    ...
    while (g_app_running) {
        ...
        snprintf(reg_frame, sizeof(reg_frame),
                 "arapibus %s/* * * production proxy=http://127.0.0.1:%d",
...
int main(int argc, char **argv) {
    ...
    alri_print(BLD GRN "== [ARAPIBUS] Starting Sovereign Corporate Management Backend ==\n" RST);

    arapibus_config_load("arapibus.cfg");
    ArapibusConfig *cfg = arapibus_config_get();

    if (arapibus_db_init(cfg->data_dir) != 0) {
        alri_print(RED "[ARAPIBUS] Failed to initialize corporate database\n" RST);
        return 1;
    }

    if (arapibus_http_server_start(cfg->server_bind, cfg->server_port) != 0) {
        alri_print(RED "[ARAPIBUS] Failed to start HTTP server on port %d\n" RST, cfg->server_port);
        arapibus_db_close();
        return 1;
    }

    void *ipc_t = ar_thread_create(arapibus_ipc_thread, NULL);

    while (g_app_running) {
        ar_sleep_ms(250);
    }

    alri_print(YLW "[ARAPIBUS] Shutting down service...\n" RST);
    arapibus_http_server_stop();
    arapibus_db_close();

    if (ipc_t) ar_thread_join(ipc_t);

    alri_print(GRN "[ARAPIBUS] Clean shutdown completed.\n" RST);
    return 0;
}
