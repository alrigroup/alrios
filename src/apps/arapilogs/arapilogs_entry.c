...
static void handle_ipc_query(int fd, const char *payload, int payload_len) {
    ...
    if (strcmp(q, "status") == 0) {
        rlen = snprintf(resp, sizeof(resp),
            "[ARAPILOGS] Audit & Telemetry API Backend Status:\n"
            "  State: RUNNING\n"
            "  HTTP Port: %d (Active)\n"
            "  Route Prefix: %s\n"
            "  Storage: %s\n"
            "  DM Isolation: ACTIVE (Guaranteed Private)\n",
            cfg->server_port, cfg->route_prefix, cfg->data_dir);
    } else {
        rlen = snprintf(resp, sizeof(resp),
            "ARAPILOGS — Sovereign Audit & Telemetry API Backend v1.0.0\n"
            "  status - View backend status and ingestion engine state\n");
    }

    ar_ipc_send_frame(fd, IPC_QUERY_RESP, resp, (uint32_t)rlen + 1);
}

static void *arapilogs_ipc_thread(void *arg) {
    (void)arg;
    ArapilogsConfig *cfg = arapilogs_config_get();
    ...
    while (g_app_running) {
        ...
        snprintf(reg_frame, sizeof(reg_frame),
                 "arapilogs %s/* * * production proxy=http://127.0.0.1:%d",
...
int main(int argc, char **argv) {
    ...
    alri_print(BLD GRN "== [ARAPILOGS] Starting Sovereign Audit & Telemetry Backend ==\n" RST);

    arapilogs_config_load("arapilogs.cfg");
    ArapilogsConfig *cfg = arapilogs_config_get();

    if (arapilogs_db_init(cfg->data_dir, cfg->max_logs) != 0) {
        alri_print(RED "[ARAPILOGS] Failed to initialize audit database\n" RST);
        return 1;
    }

    if (arapilogs_http_server_start(cfg->server_bind, cfg->server_port) != 0) {
        alri_print(RED "[ARAPILOGS] Failed to start HTTP server on port %d\n" RST, cfg->server_port);
        arapilogs_db_close();
        return 1;
    }

    void *ipc_t = ar_thread_create(arapilogs_ipc_thread, NULL);

    while (g_app_running) {
        ar_sleep_ms(250);
    }

    alri_print(YLW "[ARAPILOGS] Shutting down service...\n" RST);
    arapilogs_http_server_stop();
    arapilogs_db_close();

    if (ipc_t) ar_thread_join(ipc_t);

    alri_print(GRN "[ARAPILOGS] Clean shutdown completed.\n" RST);
    return 0;
}
