...
static ArapilogsConfig g_cfg;
static int g_loaded = 0;

static void set_defaults(void) {
    memset(&g_cfg, 0, sizeof(g_cfg));
    strncpy(g_cfg.app_id, "arapilogs", sizeof(g_cfg.app_id) - 1);
    strncpy(g_cfg.app_name, "ALRI Sovereign Audit & Telemetry API Backend", sizeof(g_cfg.app_name) - 1);
    g_cfg.server_port = 9655;
    strncpy(g_cfg.server_bind, "127.0.0.1", sizeof(g_cfg.server_bind) - 1);
    strncpy(g_cfg.route_prefix, "/arapi/logs", sizeof(g_cfg.route_prefix) - 1);
    strncpy(g_cfg.auth_host, "127.0.0.1", sizeof(g_cfg.auth_host) - 1);
    g_cfg.auth_port = 9650;
    strncpy(g_cfg.data_dir, "storage/arlogs", sizeof(g_cfg.data_dir) - 1);
    g_cfg.max_logs = 5000;
}
...
int arapilogs_config_load(const char *path) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        } else if (strcmp(key, "max_in_memory_logs") == 0) {
            g_cfg.max_logs = atoi(val);
        }
    }
