...
static void handle_ipc_query(int fd, const char *payload, int payload_len) {
    ...
    if (strcmp(q, "status") == 0) {
        rlen = snprintf(resp, sizeof(resp),
            "[ARAPICHAT] Support & Tickets API Backend Status:\n"
            "  State: RUNNING\n"
            "  HTTP Port: %d (Active)\n"
            "  Route Prefix: %s\n"
            "  Storage: %s\n",
        ...
        rlen = snprintf(resp, sizeof(resp),
            "ARAPICHAT — Sovereign Support & Tickets API Backend v1.0.0\n"
...
static void *arapichat_ipc_thread(void *arg) {
    (void)arg;
    ArapichatConfig *cfg = arapichat_config_get();
    ...
    while (g_app_running) {
        ...
        snprintf(reg_frame, sizeof(reg_frame),
                 "arapichat %s/* * * production proxy=http://127.0.0.1:%d",
...
int main(int argc, char **argv) {
    ...
    alri_print(BLD GRN "== [ARAPICHAT] Starting Sovereign Support & Tickets Backend ==\n" RST);

    arapichat_config_load("arapichat.cfg");
    ArapichatConfig *cfg = arapichat_config_get();

    if (arapichat_db_init(cfg->data_dir) != 0) {
        alri_print(RED "[ARAPICHAT] Failed to initialize support database\n" RST);
        return 1;
    }

    if (arapichat_http_server_start(cfg->server_bind, cfg->server_port) != 0) {
        alri_print(RED "[ARAPICHAT] Failed to start HTTP server on port %d\n" RST, cfg->server_port);
        arapichat_db_close();
        return 1;
    }

    void *ipc_t = ar_thread_create(arapichat_ipc_thread, NULL);

    while (g_app_running) {
        ar_sleep_ms(250);
    }

    alri_print(YLW "[ARAPICHAT] Shutting down service...\n" RST);
    arapichat_http_server_stop();
    arapichat_db_close();

    if (ipc_t) ar_thread_join(ipc_t);

    alri_print(GRN "[ARAPICHAT] Clean shutdown completed.\n" RST);
    return 0;
}
